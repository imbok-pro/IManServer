...
--  
-------------------------------------------------------------------------------
CREATE OR REPLACE VIEW SP.VG_OBJECTS
...
where g.ID = o.GROUP_ID 
  and g.ID in (select * from table (SP.GRAPH2TREE.SelectNodes('ObjectsGroup'))
               union
               select SP.Graph2Tree.GetRoot('ObjectsGroup') from dual
               )
...
grant all on SP.VG_OBJECTS to public;
Comment on table SP.VG_OBJECTS is 'Объекты, включенные в группу, определяемую глобальным параметром ObjectsGroup. SP-GObjects-Views.vw';
...
Comment on column SP.VG_OBJECTS.KIND is 'Описание вида объекта.';
Comment on column SP.VG_OBJECTS.LINES is 'Число втрок в макропроцедуре.';
Comment on column SP.VG_OBJECTS.USING_ROLE is 'Роль, которую должен иметь пользователь, чтобы иcпользовать объект в элементах своих объектов. Пользователь, имеющий SP_ADMIN_ROLE, может использовать любой объект. Если поле нулл, то объект публичен.';
Comment on column SP.VG_OBJECTS.EDIT_ROLE is 'Роль, которую должен иметь пользователь дополнительно к SP_DEVELOPING_ROLE, чтобы изменять объект, а также добавлять удалять или изменять его параметры. Пользователь, имеющий SP_ADMIN_ROLE, может изменять любой объект. Если поле нулл, то только администратор может изменять объект.';
